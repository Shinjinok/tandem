 ...
 */
/*
  suppport for FlightGear MIP parsing
 */
...
protected:
    ...
    enum class ParseState {
        ...
        // A FlightGear packet can be a maximum of 261 bytes
    struct FlightGear_Packet {
        uint8_t header[4];
        uint8_t payload[255];
        uint8_t checksum[2];
    };
    ...
    struct {
        FlightGear_Packet packet;
        AP_FlightGear::ParseState state;
        uint8_t index;
    } message_in;
    ...
    static bool valid_packet(const FlightGear_Packet &packet);
    // Calls the correct functions based on the packet descriptor of the packet
    DescriptorSet handle_packet(const FlightGear_Packet &packet);
    // Collects data from an imu packet into `imu_data`
    void handle_imu(const FlightGear_Packet &packet);
    // Collects data from a gnss packet into `gnss_data`
    void handle_gnss(const FlightGear_Packet &packet);
    void handle_filter(const FlightGear_Packet &packet);
